...
z_alfa = stats.t.ppf(1-alfa, (n-1))           # для одностороннего правого критерия
...
d_sample = np.std(a_sample, ddof=1)
t = (m_sample - m_0) / np.sqrt(d_sample/n)      # Определяем критерий Стьюдента, т.к. нет данных ген. совокупности
z_alfa_r = stats.t.ppf(alfa, (n-1))             # для правостороннего тестирования
z_alfa_l = stats.t.ppf(1-alfa, (n-1))           # для левостороннего тестирования

print()
print('Задание-3.')
print('--------------------------------')
print(f'Среднее по выборке из {n} пачек: Mean = {m_sample}')
print(f'Дисперсия по выборке из {n} пачек: D = {d_sample}')
...
print(f'рост матерей: {g_mother}', f'рост взрослых дочере: {g_daug}', sep='\n')
